...
namespace ConsoleUI
{
    ...
    {
        /* This is a beter away of doing things. 'T' is the generic value passed in as a generic. It could be anything
         Right here, we are saying, whatever passed in that is a class, and have the ability to have an empty constructor,(new())*/
        public static List<T> LoadFromTextFile<T>(string filePath) where T : class, new()
        {
            var lines = System.IO.File.ReadAllLines(filePath).ToList();
            //We don't know what the type is for 'T'
            List<T> output = new List<T>();
            T entry = new T(); //We create a new instance of T, with an empty contructor. We can't do this unless we have the requirement of 'new' above
            var cols = entry.GetType().GetProperties(); //This is a reflection, a list of properties from a class. Kind of performance hard
            ...
            foreach (var row in lines)
            {
                ...
                for (var i = 0; i < headers.Length; i++)
                {
                    foreach (var col in cols)
                    {
                        ...
                        {
                            /* Whatever value goes into that row, (string, boolean, etc.), we are converting the value into that type*/
